...
def default_mapping(atom_number):
    return atom_number


def write_lmpdat(infile, outfile, scale_pos=10.0, mapping=default_mapping):
    """Writes input file in LAMMPS data format for chemsim.

    Args:
        infile: Input file readable by mdtraj.
        outfile: Output file name.
        scale_pos: Scale factor for positions. Default 10.0 corresponds to
            LAMMPS unit style real.
        mapping: Function to convert atom number to species.
...
def convert_lammpstrj(topology, lammpstrj, outfile, frame=None, stride=None):
    """Convert LAMMPS trajectory to MDTraj format."""
    top = mdtraj.load_topology(topology)
    traj = mdtraj.load_lammpstrj(lammpstrj, top, frame=frame, stride=stride)
    traj.save(outfile)
